            ...
            <v-col cols="6">
              <v-card class="elevation-4">
                <div class="d-flex pa-3">
                  ...
                  <v-spacer></v-spacer>
                  <v-btn class="mb-4 justify-end" color="primary" @click="showAddDialog('physioPositioning')">Add New Option</v-btn>
                </div>
                <v-table>
                        <thead>
                        <tr>
                        ...
                        <tbody class="">
                        <tr
                            v-for="item in physioPositioningOptions"
                            :key="item.id"
                        >
                            <td>{{ item.text }}</td>
                            <td class="d-flex justify-end">
                              <v-icon class="ma-2 pa-3 pt-5" @click="editTreatmentOptionItem(item)">mdi-pencil</v-icon>
                    ...
                    <v-dialog v-model="showAddTreatmentOptionDialog" max-width="600px">
                      <v-card>
                        ...
                        <v-card-text>
                          <v-text-field v-model="newTreatmentOption.text" label="Treatment Option Name"></v-text-field>
                        </v-card-text>
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn color="blue darken-1" text @click="showAddTreatmentOptionDialog = false">Cancel</v-btn>
                          <v-btn color="blue darken-1" text @click="addTreatmentOption(currentTreatmentOptionCategory)">Save</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
                    ...
                    <v-dialog v-model="showEditTreatmentOptionDialog" max-width="600px">
                      <v-card>
                        ...
                        <v-card-text>
                          <v-text-field v-model="editedTreatmentOption.text" label="Treatment Option Name"></v-text-field>
                        </v-card-text>
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn color="blue darken-1" text @click="showEditTreatmentOptionDialog = false">Cancel</v-btn>
                          <v-btn color="blue darken-1" text @click="updateTreatmentOption">Update</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>

                    <v-dialog v-model="showDeleteTreatmentOptionDialog" max-width="600px">
                      <v-card>
                        <v-card-title>
                        ...
                        <v-card-actions>
                          <v-spacer></v-spacer>
                          <v-btn color="blue darken-1" text @click="showDeleteTreatmentOptionDialog = false">No</v-btn>
                          <v-btn color="red darken-1" text @click="deleteTreatmentOption">Yes</v-btn>
                        </v-card-actions>
                      </v-card>
                    </v-dialog>
              </v-card>
            ...
            <v-col cols="6">
              <v-card class="elevation-4">
                <div class="d-flex pa-3">
                  ...
                  <v-spacer></v-spacer>
                  <v-btn class="mb-4 justify-end" color="primary" @click="showAddDialog('treatmentPositioning')">Add New Option</v-btn>
                </div>
                <v-table>
                        <thead>
                        <tr>
                        ...
                        <tbody class="">
                        <tr
                            v-for="item in treatmentPositioningOptions"
                            :key="item.id"
                        >
                            <td>{{ item.text }}</td>
                            <td class="d-flex justify-end">
            ...
            <v-col cols="6">
              <v-card class="elevation-4">
                <div class="d-flex pa-3">
                  ...
                  <v-spacer></v-spacer>
                  <v-btn class="mb-4 justify-end" color="primary" @click="showAddDialog('treatmentTechnique')">Add New Option</v-btn>
                </div>
                <v-table>
                        <thead>
                        <tr>
                        ...
                        <tbody class="">
                        <tr
                            v-for="item in treatmentTechniqueOptions"
                            :key="item.id"
                        >
                            <td>{{ item.text }}</td>
                            <td class="d-flex justify-end">
...
export default {
  ...
  methods: {
    async loadTreatmentOptions() {
      try {
        this.treatmentOptions = await this.treatmentOptionService.getTreatmentOptions();
        this.physioPositioningOptions = this.treatmentOptions.filter((option) => option.category === 'physioPositioning');
        this.treatmentPositioningOptions = this.treatmentOptions.filter((option) => option.category === 'treatmentPositioning');
        this.treatmentTechniqueOptions = this.treatmentOptions.filter((option) => option.category === 'treatmentTechnique')
      } catch (error) {
        console.error(error);
      }
    },
    async addTreatmentOption(category) {
      try {
        ...
        await this.treatmentOptionService.addTreatmentOption(this.newTreatmentOption);
    ...
    async updateTreatmentOption() {
      try {
        await this.treatmentOptionService.updateTreatmentOption(this.editedTreatmentOption);
    ...
    async deleteTreatmentOption() {
      try {
        await this.treatmentOptionService.deleteTreatmentOption(this.treatmentOptionToDelete);
